...
@Component(immediate = true, property = { "com.liferay.portlet.display-category=category.social",
		...
		"javax.portlet.init-param.template-path=/", "javax.portlet.name=" + GuestbookPortletKeys.GUESTBOOK,
...
public class GuestbookPortlet extends MVCPortlet {
	public void addEntry(ActionRequest request, ActionResponse response) throws PortalException {
		...
		long mobile_no = ParamUtil.getLong(request, "mobile_no");
		...
		if (entryId > 0) {
			...
			try {
				...
				_entryLocalService.updateEntry(serviceContext.getUserId(), guestbookId, entryId, userName, email,
						message, mobile_no, serviceContext);
	...
	public void render(RenderRequest renderRequest, RenderResponse renderResponse)
		...
		try {
			...
			if (guestbooks.isEmpty()) {
				Guestbook guestbook = _guestbookLocalService.addGuestbook(serviceContext.getUserId(), "Aadil",
						"aadil@liferay.com", "Good Morning", serviceContext);
